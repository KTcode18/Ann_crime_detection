...
def load_raw_data():
    ...
    # Pre-parse Month here so Risk Insights can use it later without re-parsing strings
    df['_Month'] = pd.to_datetime(
        df['Time of Occurrence'], format='mixed', dayfirst=True, errors='coerce'
    ).dt.month
    return df
...
def preprocess(df: pd.DataFrame):
    d = df.drop(columns=['_Month'], errors='ignore').copy()
...
if run_training:
    ...
    with st.expander("📄 Raw Data Preview", expanded=False):
        st.dataframe(raw_df.drop(columns=['_Month']).head(20), use_container_width=True)
        st.caption(f"Shape: {raw_df.shape[0]:,} rows × {raw_df.shape[1] - 1} columns")
    ...
    tmp = raw_df.copy()
    tmp['Month_Period'] = pd.to_datetime(
    ...
    c3.metric("High-Risk Threshold", f"{mean_crime_count:.1f} crimes/city-month")
    ...
    st.write(f"Train: **{len(X_train):,}** samples · Test: **{len(X_test):,}** samples")
    ...
    # Build results_df — City and Month come from raw_df using X_test's preserved index
    results_df = pd.DataFrame({
        'Actual'    : y_test.values,
        'Predicted' : y_pred,
        'Risk Prob' : y_pred_proba,
        'City'      : raw_df.loc[X_test.index, 'City'].values,
        'Month'     : raw_df.loc[X_test.index, '_Month'].values,
    })
    ...
    st.info("👈 Adjust settings in the sidebar then click **Load Data & Train Model** to begin.")
    st.markdown("""
    ### What this app does
    ...
    4. **Trains** an ANN with configurable layers, neurons, and activation function
    5. **Evaluates** with Accuracy, Precision, Recall, F1, AUC, Confusion Matrix, and ROC Curve
    6. **Risk Insights** — High risk cities, months, and a City × Month heatmap
    """)
